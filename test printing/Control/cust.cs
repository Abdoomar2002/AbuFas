...
{
    public partial class cust : UserControl
    {
        ...
        public cust(DbContextOptions<AppDbContext> options)
        {
        ...
        public void load(List<Cust> custs = null)
        {
            CustomersList.Rows.Clear();
            var list = Program._context.Custs.Where(c => c.IsArchived == false).ToList();
            if (searchkey)
            {
        ...
        public void custLoad(int id)
        {
            ...
            if (id != 0)
            {
                var list = Program._context.CustDatas.Where(c => c.Customer.Id == id).ToList();
                if (list.Count > 0)
                {
        ...
        {
            inout.Rows.Clear();
            var list = Program._context.Custs.Where(c => c.IsArchived == true).ToList();
            if (list.Count > 0)
            {
                int i = 0;
                foreach (var item in list)
        ...
        private void sendToArchive_Click(object sender, EventArgs e)
        {
            ...
            if (msg == DialogResult.Yes)
            {
                ...
                if (bid != null)
                {
                    var custToDelete = Program._context.Custs.Where(c => c.Name == bid).FirstOrDefault();
                    ...
                    else
                    {
                        ...
                        AddCustomer.SendToBack();
                        var list = Program._context.CustDatas.Where(c => c.Customer == null).ToList();
                        Program._context.CustDatas.RemoveRange(list);
                        Program._context.SaveChanges();
                    }
                }
        ...
        private void SaveChange_Click(object sender, EventArgs e)
        {
            ...
            var cust = Program._context.Custs.Where(c => c.Name.Equals(custName.Text)).FirstOrDefault();
            if (cust == null) { cust = new Cust(); Program._context.Custs.Add(cust); }
            cust.IsArchived = false;
            cust.Name = custName.Text;
            double grams = 0, money = 0;
            var List = new List<CustData>();
            ...
            var outcoumeList = new List<CustData>();
        ...
        private void inout_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 1)
            {
                ...
                if (ms == DialogResult.Yes)
                {
                    ...
                    var item = Program._context.Custs.Where(c => c.Id == bid).FirstOrDefault();
                    if (item != null)
                    {
        ...
        private void deleteCustomer_Click(object sender, EventArgs e)
        {
            ...
            if (msg == DialogResult.Yes)
            {
                ...
                if (bid != null)
                {
                    ...
                    {
                        Program._context.Custs.Remove(custToDelete);
                        Program._context.SaveChanges();
